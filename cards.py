...
def generate_scenario(players_count: int) -> dict:
    """Generate a varied 2-player contract scenario.

    Every scenario has TWO layers: a total budget for the final pair and a
    second football requirement. The budget is deliberately independent from
    the secondary requirement so scenarios are less repetitive and the AI has
    a clear, achievable target to judge.
    """
    club = random.choice(CLUBS)
    winners_needed = 2

    # Budget belongs to BOTH finalists together, not to each player.
    # Wide range keeps the market requirement from feeling predictable.
    budget = random.choice([80, 100, 110, 120, 130, 140, 150, 160, 180, 200, 220, 250, 280, 320, 350])

    templates = [
        ("attacker", "⚡ <b>ДВА УДАРА В ОДНО ОКНО</b>", "минимум один из пары должен быть способен решать эпизоды в завершении: сильный удар, хладнокровие 1 в 1 или уверенная игра первым касанием"),
        ("attacker", "🚀 <b>КОНТРАТАКА БЕЗ ТОРМОЗОВ</b>", "минимум один игрок должен подходить под быстрый футбол: взрывной рывок, скорость с мячом или качественное открывание за спину"),
        ("attacker", "🦊 <b>ОХОТА В ШТРАФНОЙ</b>", "в паре нужен хотя бы один игрок с выраженным инстинктом завершителя: игра в одно касание, тайминг открываний или хладнокровное завершение"),
        ("attacker", "🧱 <b>ВЗЛОМ НИЗКОГО БЛОКА</b>", "пара должна дать хотя бы один способ вскрывать плотную оборону: дриблинг, нестандартный пас или сильная игра на ограниченном пространстве"),
        ("attacker", "🔥 <b>ПОСЛЕДНИЕ 20 МИНУТ</b>", "минимум один игрок должен быть пригоден для решающего штурма: сильный удар, игра на втором этаже или опасность в штрафной"),
        ("defender", "🛡 <b>ЗАКРЫТЬ МАТЧ</b>", "минимум один из двух должен давать явную оборонительную надежность: подкаты, блокировки, верховые дуэли или выбор позиции"),
        ("defender", "🧠 <b>ВЫЙТИ ИЗ-ПОД ПРЕССИНГА</b>", "в паре нужен хотя бы один игрок с качественным первым пасом, чтением игры или спокойными решениями под давлением"),
        ("defender", "🏰 <b>ОБОРОНА ПОСЛЕДНЕГО РУБЕЖА</b>", "минимум один игрок должен подходить для критических эпизодов: страховка партнеров, игра на линии ворот или сильная игра в верховых дуэлях"),
        ("defender", "⚔️ <b>ЖЕСТКИЙ ВЫЕЗД</b>", "в паре нужен хотя бы один физически и психологически подходящий защитник: силовой отбор, борьба или уверенность в жестких стыках"),
        ("defender", "🎯 <b>НЕ ДАТЬ ЗВЕЗДЕ ДЫШАТЬ</b>", "хотя бы один игрок должен быть пригоден для персональной опеки, чтения игры или своевременных перехватов"),
        ("midfielder", "⚙️ <b>ВЕРНУТЬ ЦЕНТР</b>", "минимум один игрок должен уметь контролировать темп: видение поля, скрытые передачи, футбольный IQ или способность задавать ритм"),
        ("midfielder", "🧩 <b>СВЯЗАТЬ КОМАНДУ</b>", "в паре нужен хотя бы один игрок, способный соединять линии: качественный пас, движение между линиями или хорошее чтение игры"),
        ("midfielder", "🔋 <b>ТЕМП ДО ФИНАЛЬНОГО СВИСТКА</b>", "минимум один игрок должен подходить под интенсивный футбол: активный прессинг, бокс-ту-бокс стиль или работа без мяча"),
        ("midfielder", "🎭 <b>ПЛАН Б НА 70-Й МИНУТЕ</b>", "пара должна иметь хотя бы один нестандартный инструмент: дриблинг, дальние передачи, стандарты или умение менять темп"),
        ("midfielder", "👁 <b>УВИДЕТЬ ТО, ЧЕГО НЕ ВИДЯТ ДРУГИЕ</b>", "минимум один игрок должен давать креатив: видение поля, скрытые передачи, плеймейкерские качества или нестандартное решение"),
    ]

    line, title, requirement = random.choice(templates)
    # Narrative variants multiply the combinations without changing rules.
    intro = random.choice([
        f"В {club.upper()} открылось трансферное окно и клуб собирает новую связку.",
        f"После провального матча {club.upper()} требует срочно собрать рабочую пару игроков.",
        f"Спортивный директор {club.upper()} получил один шанс на двойной трансфер.",
        f"Тренер {club.upper()} меняет план на сезон и просит подписать ровно двух футболистов.",
        f"Перед важнейшим матчем {club.upper()} ищет двух игроков под конкретную задачу.",
    ])
    twist = random.choice([
        "Допускаются разные роли: важнее, чтобы два игрока вместе закрывали задачу.",
        "Не требуется идеальная карточка — сильные стороны одного могут дополнять другого.",
        "Возраст сам по себе не является причиной отказа, если игрок полезен задаче.",
        "Второстепенные минусы допустимы, если ключевое требование реально закрыто.",
        "Клуб оценивает пару как связку, а не требует, чтобы оба были одинаково хороши во всем.",
    ])

    text = (
        f"{title}\n{intro}\n\n"
        f"📋 <b>КОНТРАКТ НА ДВОИХ:</b>\n"
        f"💰 <b>Общий бюджет пары: до €{budget}M</b>\n"
        f"🎯 <b>Дополнительное требование:</b> {requirement.capitalize()}.\n\n"
        f"ℹ️ {twist}"
    )
    ...
    return {
        ...
        "line": line,
        "text": text,
        "price_requirement": budget,
        "budget_for_pair": budget,
        "market_event": None,
    }
